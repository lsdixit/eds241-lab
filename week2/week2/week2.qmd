...
counts_gpx <- read_csv(here::here("week2", "week2", "data", "waste_pile_counts_gpx.csv")) %>% 
...
```
Intercept: without treatment, we expect the number of treat piles to be ~78 on average.
...
```
Intercept: on average, the model estimates our control to have 78 waste piles.
...
m2_change = (exp(-.38) - 1) * 100 # change in poisson # -31.6
...
```
Intercept: when controlling for pre-count, untreated neighborhoods had an average of 2 waste piles.
Treatment: reduction of an avergae 24% waste piles when controlling for pre-count average
pre-count: 
...
export_summs(m3_log, m4_control,
             model.names = c("Log-OLS", "Log-OLS w/ Control Variable"),
             statistic = "none")
```
SE for treatment was halved, which makes sense when controlling for variables that may be affecting the treatment influence.
