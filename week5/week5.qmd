...
sim_data <- read_csv(here("data", "Simulated_Deforestation_Data6.csv"), show_col_types = FALSE)
```
...
**RESPONSE**: These two lines represent deforestation rate in protected and non-protected areas. The jump at 0 from high to low deforestation represents the treatment effect of protected areas. Positive values of X are distance into a protected area from the border, and negative values are the distance out of a protected area from the border. If there was no discontinuity, then the trend would be consistent with no jump at 0. If there was no jump, we would assume no treatment effect from protected areas on deforestation rate.
...
rdd_ols <- lm(
  deforest ~ # outcome
    ...
    protected * distance + # interaction to allow slopes to vary
...
**RESPONSE**: Protected areas reduce deforestation by on average 0.1. Yes it matches our plot! The interaction coefficient represents the difference in slopes (protected area slope is 0.002 higher)
...
**RESPONSE**: Because we have a bandwidth, we are using a much smaller subset of samples compared to the OLS where we are using all of our data. This analysis accounts for weights of each point, so points closer to the point weigh more in the analysis. Because this is two separate non-linear regressions, points fall closer together on the border and change the value of the coefficient slightly. 
...
**RESPONSE**: The treatment effect at 0 is much smaller in the local polynomial plot, and the second plot better captures the pattern data as it is not straight.
...
run_country_rdd <- function(country_name) {
  # filter data just for input country
  df_country <- sim_data %>% filter(country == country_name)
  
  # build model to run for that country
  rdd_model <- rdrobust(
...
**RESPONSE**: There are differences in discontinuity between our simulated data and Neal 2024, and differences in smoothing.
