...
tbl_3way <- lobsters %>% 
  ...
  summarise(total_cpue = sum(cpue, na.rm = TRUE),
            .groups = "drop") %>% 
  # pivot wider to create column for each treatment/control group within each region
  pivot_wider(
...
table_w_plots <- lobsters %>% 
  ...
  summarise(
    total_cpue = sum(cpue, na.rm = TRUE),
    dist_cpue = list(cpue), # store all indv cpue values as a list for each year
    .groups = "drop"
  ) %>% 
  arrange(year) %>% 
  # create gt table
  ...
  gtExtras::gt_plt_dist(
    dist_cpue, "density", line_color = "blue", fill_color = "red"
  )
...
library(cowsay)
library(beepr)
say("All done making some beautiful tables! :) ", "random"); beep(3)
