...
*Response:* pollution risk low, reef exposure
...
*Response:* If pollution risk is lower in MPA's before the experiment begins, then we would have a hard time attributing causal effects purely due to treatment. Exposed reefs may be experiencing higher nutrients, being nearer to mangroves may increase fish populations as mangroves foster higher fish populations naturally.
...
*Response:* Average treatment effect of MPAs on fish biomass. How would fish biomass from MPA sites change if these were non-MPA sites.
...
match_model <- matchit(
  treated_mpa ~ dist_to_deep_water + ssta_freq + reef_exposure + reef_slope + 
    reef_type + dist_mangroves + dist_fish_settl + dist_market + pollution_risk + 
    monsoon_direction,
  data = data_clean, 
  method = "nearest",
...
*Response:* Our model predicts MPA sites will have an average ~80 more fisheries biomass compared to non-mpa sites.

**Q8.** What is the biggest remaining threat to causal interpretation of regression coefficients after matching?

*Response:* The matching methods controls for observed differences, but still leaves unmatched unobserved selection bias.
...
*Response:* noFEmodel = 0.126, FEmodel = 0.227. The coefficient increased by almost double, meaning our fixed effects (plot and year) were potentially adding bias into (confounding) our vpd levels. 
...
*Response:* El nino, pest outbreaks.
...
*Response:* Soil nutrients, distance to environmental factors (close to streams, etc).
