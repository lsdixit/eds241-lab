...
**RESPONSE:** Winter months increasing pollution and respiratory illness, socioeconomic factors affecting pollution and respiratory illness.
...
```{r}
options(noaakey = "fScDyxjuOVzKTZfXIfqCMVjZjKvaYKQB")
```
...
High standard error, potential outliers
...
ggplot(data_weekly_joined, aes(x = pm25_mean, y = asthma_index)) +
  ...
  labs(x = "Weekly PM2.5 (AQI)",
       y = "Asthma Inhaler Search Index", 
       title = "PM2.5 vs. Asthma Inhaler Searches") +
  theme_minimal()
```
...
ggplot(data_weekly_joined, aes(x = precip_total, y = pm25_mean)) +
  geom_point(alpha = 0.6, color = "darkgreen", size = 2) +
  geom_smooth(method = "lm", se = TRUE, color = "dodgerblue", linewidth = 1) +
  labs(x = "Weekly Total Precipitation (inches)",
       y = "Weekly PM 2.5 (AQI)", 
       title = "Precipitation vs. PM2.5 (First stage relationship)") +
  theme_minimal()
```
...
**RESPONSE:** negative, 1 inch of precip decreased pm2.5 by 0.1 which does make sense. WEAAAAK, lots of unexplained variance.
...
**RESPONSE:** Worse air means you are inside and able to search more. Increase in mold from increased moisture leads to more respiratory issues. Precipitation would no longer be an IV, but a direct predictor. 
...
**RESPONSE:** Increase observations, look at multiple cities
...
**RESPONSE:** Wind, wildfire smoke, thermal inversions
...
**RESPONSE:** Exclusion requirement, relevance, independence, monotenicity 
