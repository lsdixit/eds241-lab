...
#| message: FALSE
#| warning: false
LA_2019 <- read_csv(here::here("week3", "week3", "data",  "LA_2019.csv")) %>%
  filter(`Local Site Name` == "West Los Angeles") %>% mutate(city = "LA", year = 2019)


LA_2020 <- read_csv(here::here("week3", "week3", "data", "LA_2020.csv")) %>% 
  filter(`Local Site Name` == "West Los Angeles") %>% mutate(city = "LA", year = 2020)

Phoenix_2019 <- read_csv(here::here("week3", "week3","data", "phoenix_2019.csv")) %>% 
  filter(`Local Site Name` == "WEST PHOENIX") %>% 
  mutate(city = "Phoenix", year = 2019)

Phoenix_2020 <- read_csv(here::here("week3", "week3", "data", "phoenix_2020.csv")) %>% 
  filter(`Local Site Name` == "WEST PHOENIX") %>% 
...
Because of the large spike in LA before march, it violates the parrallel trends assumption and we cannot confidently attribute any differences to the treatment.
...
NM_2019 <- read_csv(here::here("week3", "week3", "data", "nm_2019.csv")) %>% mutate(state = "NM", year = 2019, 
      `County FIPS Code`  = as.numeric(`County FIPS Code`))                                                                                   

NM_2020 <- read_csv(here::here("week3", "week3", "data", "nm_2020.csv")) %>%  mutate(state = "NM", year = 2020,                       `County FIPS Code`  = as.numeric(`County FIPS Code`))

TX_2019 <- read_csv(here::here("week3", "week3", "data", "tx_2019.csv")) %>% 
  mutate(state = "TX", year = 2019)

TX_2020 <- read_csv(here::here("week3", "week3", "data", "tx_2020.csv")) %>% 
...
```
our predicts that NM received an average 3.5 less ppb of no2 per hour per week when compared to TX
...
It's harder for a model to pick up on trends when there is a lot of fluctuation
...
Not significant, we could get this difference in treatment effect randomly by chance.
...
El Paso, next to each other but in different states. Nearby conditions similar, but different mandates for lockdown. If the dust storm hit both then no.
